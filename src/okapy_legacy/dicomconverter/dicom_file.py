...
from okapy_legacy.dicomconverter.seg_highdicom_adapter import read_seg_with_highdicom

from okapy_legacy.dicomconverter.volume import Volume, BinaryVolume, ReferenceFrame
from okapy_legacy.dicomconverter.dicom_header import DicomHeader
from okapy_legacy.exceptions import (
    EmptyContourException,
...
class DicomFileBase:
    ...
    @staticmethod
    def from_dicom_slices(
        dicoms, study=None, additional_dicom_tags=None, submodalities=False
    ):
    ...
    def __init__(
        ...
        if submodalities:
            self.additional_dicom_tags = (
                ["SeriesDescription"]
                if additional_dicom_tags is None
                else additional_dicom_tags + ["SeriesDescription"]
            )
        else:
    ...
    def dicom_header(self):
        if self._dicom_header is None and not type(self.dicom_paths[0]) == FileDataset:
            self._dicom_header = DicomHeader.from_file(
                self.dicom_paths[0], additional_tags=self.additional_dicom_tags
            )

        elif self._dicom_header is None and type(self.dicom_paths[0]) == FileDataset:
    ...
    def patient_weight(self):
        ...
        if patient_weight is None:
            raise MissingWeightException("Weight is missing in {}".format(self))
...
class DicomFileImageBase(DicomFileBase, name="image_base"):
    ...
    def _check_shape_consistency(
        self, slices, dicom_paths, orthogonal_positions, dimension="Columns"
    ):
        ...
        ind2rm = [ind for ind in range(len(slices)) if slices[ind].Columns in val[:-1]]

        if len(ind2rm) > 0:
            slices = [k for i, k in enumerate(slices) if i not in ind2rm]
            dicom_paths = [k for i, k in enumerate(dicom_paths) if i not in ind2rm]
            orthogonal_positions = [
                k for i, k in enumerate(orthogonal_positions) if i not in ind2rm
            ]
    ...
    def _check_redundancy(self, slices, dicom_paths, orthogonal_positions):
        ...
        ind2rm = [
            ind
            for ind in range(len(orthogonal_positions))
    ...
    def _check_missing_slices(self):
        ...
        slice_discontinuities = (
            np.abs(self.d_slices - self.slice_spacing) > 0.9 * self.slice_spacing
        )
        n_missing_slices = np.round(
            np.sum(self.d_slices[slice_discontinuities] / self.slice_spacing - 1)
        ).astype(int)
    ...
    def read(self):
        ...
        (slices, dicom_paths, orthogonal_positions) = self._check_shape_consistency(
    ...
    def _interp_missing_slice(self, image):
        ...
        new_slice = (image[:, :, ind2interp] + image[:, :, ind2interp + 1]) * 0.5
        new_slice = new_slice[..., np.newaxis]
        image = np.concatenate(
            (image[..., :ind2interp], new_slice, image[..., ind2interp:]), axis=2
        )
        return image
    ...
    def get_volume(self):
        ...
        if self.n_missing_slices == 1:
            logger.warning(
                f"One slice is missing, we replace "
        ...
        if np.sum(self.slice_discontinuities) == 1 and self.n_missing_slices > 1:
            logger.warning(
                f"Multiple slices are missing for patient"
                f"{self.dicom_header.PatientID}"
                f" and modality "
                f"{self.dicom_header.Modality} "
...
class DicomFileCT(DicomFileImageBase, name="CT"):
    ...
    def get_physical_values(self):
        if not hasattr(self.slices[0], "pixel_array"):
            ...
            image = [DicomFileCT._get_physical_per_slice(s) for s in self.slices]
...
class DicomFilePT(DicomFileImageBase, name="PT"):
    ...
    def patient_weight(self):
        ...
        try:
            ...
            list_images = [
                f
                for f in self.study.volume_files + self.study.mask_files
                if f is not self
            ]
            weight_found = False
            ...
            if not weight_found:
                logger.warning(
                    f"Estimation of patient weight by 75.0 kg"
    ...
    def get_physical_values(self):
        ...
        if not hasattr(self.slices[0], "pixel_array"):
            image = [phys_value_func(pdcm.dcmread(p)) for p in self.dicom_paths]
        else:
    ...
    def _get_decay_time(self):
        ...
        try:
            serie_datetime = datetime.strptime(
                s[0x00080021].value + s[0x00080031].value.split(".")[0], "%Y%m%d%H%M%S"
            )
        except KeyError:
            logger.warning(
        ...
        logger.debug(
            f"Computed decay time for patient "
...
class SegFile(MaskFile, name="SEG"):
    ...
    def reference_image(self):
        ...
        if self._reference_image is None:
            ...
            for f in self.study.volume_files:
                if self.reference_image_uid == f.dicom_header.SeriesInstanceUID:
                    self._reference_image = f
                    found = True
                    break

            if not found:
                self.study.volume_files.sort(key=lambda x: x.dicom_header.Modality)
    ...
    def read(self):
        ...
        self._reference_image_uid = dcm.ReferencedSeriesSequence[0].SeriesInstanceUID

        self.raw_volume = read_seg_with_highdicom(dcm)
        ...
        for segment_number in self.raw_volume.available_segments:
            label = self.raw_volume.segment_infos[segment_number]["label"]
            self._labels.append(label)
            self.label_to_num[label] = segment_number

        first_segment = self.raw_volume.available_segments[0]
        shape = self.raw_volume.segment_data(first_segment).shape
...
class RtstructFile(MaskFile, name="RTSTRUCT"):
    def __init__(self, *args, reference_image=None, reference_frame=None, **kwargs):
        super().__init__(*args, **kwargs)
        self._reference_frame = reference_frame
        self._reference_image = reference_image
        self._reference_image_uid = None
    ...
    def get_reference_image_uid(dcm):
        try:
            return (
                dcm.ReferencedFrameOfReferenceSequence[0]
    ...
    def read(self):
        self._labels = list()
        if len(self.dicom_paths) > 1:
            logger.warning("there is multiple instances of the same RTSTRUCT file")
        if type(self.dicom_paths[0]) == FileDataset:
            self.slices = self.dicom_paths
        else:
            self.slices = [pdcm.dcmread(p) for p in self.dicom_paths]
        self._reference_image_uid = RtstructFile.get_reference_image_uid(self.slices[0])
        self.label_number_mapping = {}
        for dcm in self.slices:
            if self.reference_image_uid != RtstructFile.get_reference_image_uid(dcm):
                raise RuntimeError(
            ...
            for i, roi_seq in enumerate(dcm.StructureSetROISequence):
                assert (
                    dcm.ROIContourSequence[i].ReferencedROINumber == roi_seq.ROINumber
                )
        ...
        for s in contour_sequence:
            ...
            if len(rr) > 0 and len(cc) > 0:
                if (
                    ...
                    raise Exception(
                        f"The RTSTRUCT file is compromised, "
    ...
    def get_volume(self, label):
        ...
        try:
            contour_sequence = (
                self.slices[0]
                .ROIContourSequence[self.label_number_mapping[label]]
                .ContourSequence
            )
        except AttributeError:
            logger.warning(

...
def next_batch(num, data, labels,is_augment=False):
    ...
    idx = idx[:num]
    # randomly generate rotation  (90 degrees or all from 0 to 270)
    if is_augment:
        ...
        data_shuffle = [apply_affine_transform_fixed(np.squeeze(data[idx[i]]),theta_xyz=random_angles[i]) for i in range(len(idx))]
...
def maskReshape(Mask, ksize, stride):
    ...
    Mask: binary mask
    '''
    return tf.nn.avg_pool3d(Mask, ksize=[1, ksize, ksize, ksize, 1], strides=[1, stride, stride, stride, 1], padding='VALID')
